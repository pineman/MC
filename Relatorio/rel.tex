...
\subsection*{a)}
	...
	\begin{align*} % split não permite page breaks
		...
		w = \frac{-23.75 \pm \sqrt{(-23.75)^2 - 4 * 1 * (-1)}}{2 * 1}
		\Leftrightarrow \\
		&\Leftrightarrow
...
\subsubsection*{ii.)}
	...
	$$ k_{\infty} = \lim_{n \to \infty} \frac{|z - x_{n+1}|}{|z-x_n|^p} = \frac{|g^{(p)}(z)|}{p!}$$
	$$K_\infty = |g'(\lambda_n)| = 0.0761501943658923$$

	\par A aproximação do coeficiente assimptótico de convergência obtido é
	$0.082426400103842$, apresentando um erro considerável em relação ao valor
	teórico. Tal deverá originar do facto de utilizarmos um valor
	aproximado da raiz com uma imprecisão de $\pm 10^6$.
...
\par{Sucessão de erros}
	\lstinputlisting{../src/sucessao_de_erros.m}

\par{Função iteradora \eqref{2a}}
	\lstinputlisting{../src/g.m}

\par{Função \eqref{init}}
	\lstinputlisting{../src/f.m}

\par{Derivada da função \eqref{init}}
	\lstinputlisting{../src/fderivative.m}
